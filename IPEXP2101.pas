...
// ============================================================================
//  ✅ 4. OnCopyInfoClick: ฟังก์ชัน Copy ข้อมูลเมื่อคลิกขวา
// ============================================================================
procedure TForm1.OnCopyInfoClick(Sender: TObject);
var
  S: TStringList;
  DBInfo, DBUser: string;
  IniFile: TIniFile;
begin
  S := TStringList.Create;
  try
    // พยายามดึงข้อมูลจาก Database_Log จริงๆ ก่อน (ถ้า Connect อยู่)
    if Assigned(Database_Log) and Database_Log.Connected then
    begin
       DBInfo := Format('%s:%d:%s', [Database_Log.Server, Database_Log.Port, Database_Log.Database]);
       DBUser := Database_Log.Username;
    end
    else
    begin
       // ถ้ายังไม่ Connect ให้ไปอ่านจาก INI เหมือนใน FormShow
       if FileExists(ExtractFilePath(Application.ExeName) + '/Setup/SetUp.Ini') then
       begin
         IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName) + '/Setup/SetUp.Ini');
         try
           DBInfo := IniFile.ReadString('Setting', 'DIRECTDBNAME', 'Unknown');
           DBUser := IniFile.ReadString('Setting', 'USERNAME', 'Unknown');
         finally
           IniFile.Free;
         end;
       end
    ...
    // สร้างข้อความตาม Format ที่คุณต้องการ
    S.Add('[*ENVINFO]');
    S.Add('Date: ' + FormatDateTime('yyyy/mm/dd h:nn:ss AM/PM', Now));
    S.Add('ExeName: ' + ChangeFileExt(ExtractFileName(Application.ExeName), ''));
    S.Add('ExeVersion: ' + GetAppVersion); // เรียกใช้ฟังก์ชันเดิม
    S.Add('DataArea DatabaseName: ' + DBInfo);
    S.Add('DataArea DatabaseUsername: ' + DBUser);

    Clipboard.AsText := S.Text;
    // ShowMessage('Copied Environment Info!'); // เปิดบรรทัดนี้ถ้าอยากให้แจ้งเตือน
  finally
    S.Free;
  end;
end;
...
begin
  // ตรวจสอบว่ามี Panel ครบตามที่สร้างไว้ไหม (ปกติจะมี 4 ช่องตามโค้ดข้างบน)
  if StbBase.Panels.Count >= 4 then
  begin
    // ช่องสุดท้าย (Index 3) คือช่องเวลา
    StbBase.Panels[3].Text := FormatDateTime('dd/mm/yyyy hh:nn:ss', Now);
  end;
end;
...
var
  ...
  miCopy: TMenuItem;
begin
  // สร้างเมนูคลิกขวา (Popup Menu) ด้วยโค้ด
  pmEnv := TPopupMenu.Create(Self);
  pmEnv.AutoHotkeys := maManual; // ปิด Auto Hotkey เพื่อให้โชว์เครื่องหมาย * ได้

  miCopy := TMenuItem.Create(pmEnv);
  miCopy.Caption := 'Copy Environment Info'; // ข้อความที่จะโชว์
  miCopy.OnClick := OnCopyInfoClick;         // ผูกกับฟังก์ชัน Copy ที่เราเขียนไว้
  pmEnv.Items.Add(miCopy);

  // ผูกเมนูเข้ากับ StatusBar (stbBase)
  if Assigned(stbBase) then
    stbBase.PopupMenu := pmEnv;

  // 4. โหลดค่า Setting และเชื่อมต่อ Database
  LoadSettings; // หรือ LoadSettings; ตามชื่อฟังก์ชันเดิมของคุณ
  
  try
    // เชื่อมต่อ Database (ใช้ Connection จาก qLog หรือ Database_Log ตามที่คุณใช้)
...
WSAStartup(MAKEWORD(2, 2), WSAData);
  try
    ...
    // Panel 1: Version Info (คำนวณซ้ำตาม Logic เดิมของคุณ)
...
var Sel: string;
begin
  // [JP] Select export folder -> 出力先フォルダを選択してください
  if BrowseFolder_Explorer('出力先フォルダを選択してください', Trim(edtPATH.Text), Sel) then
  ...
  try
    ...
    begin
      ...
      if BlobField.IsNull then 
        raise Exception.Create('BLOBID ' + YearStr + ' は見つかりましたが、データが空(NULL)です。');
  ...
  try
    ...
    begin
      ...
      MessageDlg('データベースからのデータ読み込みに失敗しました。' + #13#10 + 
                 '原因: ' + E.Message + #13#10 + 
                 'ログファイルを確認してください。', mtError, [mbOK], 0);
    end;
  end;
end;
...
begin
  ...
  // [JP] Blob ID not found
  raise Exception.Create('Blob ID ' + ABlobID + ' が見つかりません。'); end;
  BlobField := qSelect.FieldByName('BINARYDATA') as TBlobField;
  // [JP] Blob ID found but null
  if BlobField.IsNull then raise Exception.Create('Blob ID ' + ABlobID + ' は存在しますが、データが空(NULL)です。');
  BlobStream := qSelect.CreateBlobStream(BlobField, bmRead);
...
begin
  ...
  if Folder = '' then
  begin
    // [JP] Select export folder first
    MessageDlg('先に出力フォルダを選択してください。', mtError, [mbOK], 0);
    Exit;
  end;
  if not DirectoryExists(Folder) then
  begin
    // [JP] Export folder not found
    MessageDlg('出力フォルダが見つかりません: ' + Folder, mtError, [mbOK], 0);
    Exit;
  end;
  ...
  try
    ...
    begin
      ...
      try
        ...
        on E: Exception do
        begin
          // [JP] Year ... Failed to load data
          FailList.Add(IntToStr(Y) + '年 : データの読み込みに失敗しました。' + E.Message);
      ...
      try
        ...
        // [JP] Year ... Success
        OkList.Add(IntToStr(Y) + '年 : 成功');
      except
        on E: Exception do
        begin
          // [JP] Year ... Error
          FailList.Add(IntToStr(Y) + '年 : ' + E.Message);
    ...
    if OkList.Count > 0 then
      // [JP] Success
      Msg := Msg + '=== 成功 ===' + #13#10 + OkList.Text + #13#10 + #13#10;
    if FailList.Count > 0 then
      // [JP] Missing / Failed
      Msg := Msg + '=== 失敗 / 未検出 ===' + #13#10 + FailList.Text;
